...
class PNDMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_prk(
        ...
        if self.counter % 4 == 0:
            matrix_elementwise_multiply_by_const(model_output, 1 / 6, model_output)
            matrix_elementwise_add(self.cur_model_output, model_output, self.cur_model_output)
            self.ets.append(model_output)
            self.cur_sample = sample
        elif (self.counter - 1) % 4 == 0:
            matrix_elementwise_multiply_by_const(model_output, 1 / 3, model_output)
            matrix_elementwise_add(self.cur_model_output, model_output, self.cur_model_output)
            ...
            matrix_elementwise_multiply_by_const(model_output, 1 / 6, model_output)
            matrix_elementwise_add(self.cur_model_output, model_output, model_output)
            self.cur_model_output = 0
    ...
    def _get_prev_sample(self, sample, timestep, prev_timestep, model_output):
        ...
        sample_t = ht.empty(sample.shape, ctx=sample.ctx)
        prev_sample = ht.empty(sample.shape, ctx=sample.ctx)
        model_output_t = ht.empty(sample.shape, ctx=sample.ctx)

        if self.config.prediction_type == "v_prediction":
            # model_output = (alpha_prod_t**0.5) * model_output + (beta_prod_t**0.5) * sample
            matrix_elementwise_multiply_by_const(sample, (beta_prod_t**0.5), sample_t)
            matrix_elementwise_multiply_by_const(model_output, (alpha_prod_t**0.5), model_output)
            matrix_elementwise_add(model_output, sample_t, model_output)
